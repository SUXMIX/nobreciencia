...
const COR_CAPSULA_1 = "#1F5A3C";
const COR_CAPSULA_2 = "#C9A24B";
...
let temperaturaInicialCapsula1 = null;
let temperaturaInicialCapsula2 = null;
...
function formatarNumero(v, casas = 2) {
    if (!numeroValido(v)) return "--";
    return v.toFixed(casas).replace(".", ",");
}

function formatarTemperatura(v) {
    if (!numeroValido(v)) return "--";
    return formatarNumero(v, 2) + " °C";
}

function formatarTempo(s) {
    if (!numeroValido(s)) return "0,0 s";
    return formatarNumero(s, 1) + " s";
}
...
function atualizarInterface() {
    if (temperatura1) {
        temperatura1.innerHTML = numeroValido(temperaturaAtual)
            ? formatarNumero(temperaturaAtual, 2) + ' <small>°C</small>'
            : '-- <small>°C</small>';
    }
...
function processarTemperatura(mensagem) {
    ...
    if (etapa === "capsula2") duracaoLimite = duracaoCapsula2;
    ...
    if (tempoAtual) tempoAtual.innerHTML =
        formatarNumero(tempo, 1) + ' <small>s</small>';
...
function iniciarCapsula1() {
    ...
    temperaturaInicialCapsula1 = null;
    temperaturaInicialCapsula2 = null;
...
function pararCapsula1() {
    ...
    if (dadosCapsula1.length > 0) {
        temperaturaInicialCapsula1 = dadosCapsula1[0].temperatura;
        temperaturaFinalCapsula1   = dadosCapsula1[dadosCapsula1.length - 1].temperatura;
    }
...
function pararCapsula2() {
    ...
    if (dadosCapsula2.length > 0) {
        temperaturaInicialCapsula2 = dadosCapsula2[0].temperatura;
        temperaturaFinalCapsula2   = dadosCapsula2[dadosCapsula2.length - 1].temperatura;
    }
...
function reduzirDados(dados, maxPontos) {
    ...
    for (let i = 0; i < maxPontos; i++) {
        ...
        if (count > 0) {
            resultado.push({ tempo: somaT / count, temperatura: somaTemp / count });
        }
    }
    return resultado;
}
...
function suavizarDados(dados, raio) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        resultado[i] = { tempo: dados[i].tempo, temperatura: soma / pesoTotal };
    }
    return resultado;
}
...
function calcularEscalaY() {
    ...
    if (variacao < 0.1) {
        ...
        const m = Math.max(variacao * 0.30, 0.15);
        minimo -= m; maximo += m;
    } else if (variacao < 3) {
        const m = variacao * 0.20;
        minimo -= m; maximo += m;
    } else {
        const m = variacao * 0.10;
        minimo -= m; maximo += m;
    }

    const nv = maximo - minimo;
    let passo;
    if (nv <= 1)       passo = 0.2;
    else if (nv <= 2)  passo = 0.5;
    else if (nv <= 5)  passo = 1;
    else if (nv <= 10) passo = 2;
    else               passo = 5;
...
function desenharEixos(largura, altura, margem, tempoMaximo, escalaY) {
    ...
    for (let i = 0; i <= quantidadeLinhas; i++) {
        const p = i / quantidadeLinhas;
        const y = margem.superior + p * (altura - margem.superior - margem.inferior);
        const valor = escalaY.maximo - p * (escalaY.maximo - escalaY.minimo);
    ...
    for (let i = 0; i <= quantidadeTempos; i++) {
        const p = i / quantidadeTempos;
        const x = margem.esquerda + p * (largura - margem.esquerda - margem.direita);
        const s = p * tempoMaximo;
...
function desenharLinha(dados, margem, graficoLargura, graficoAltura,
    ...
    for (const p of dados) {
        ...
        const px = tempoMaximo > 0 ? p.tempo / tempoMaximo : 0;
        const py = (p.temperatura - temperaturaMinima) /
                   (temperaturaMaxima - temperaturaMinima);

        pontos.push({
            x: margem.esquerda + px * graficoLargura,
            y: margem.superior + graficoAltura - py * graficoAltura
        });
    }
    ...
    ctx.beginPath();
    ctx.arc(u.x, u.y, 4.5, 0, Math.PI * 2);
    ctx.fillStyle = cor;
    ctx.fill();
...
function desenharGrafico() {
    ...
    const gL = largura - margem.esquerda - margem.direita;
    const gA = altura  - margem.superior - margem.inferior;
    ...
    desenharLinha(
        prepararDadosParaDesenho(dadosCapsula1),
        margem, gL, gA, tempoMaximo,
        escalaY.minimo, escalaY.maximo, COR_CAPSULA_1
    );

    desenharLinha(
        prepararDadosParaDesenho(dadosCapsula2),
        margem, gL, gA, tempoMaximo,
        escalaY.minimo, escalaY.maximo, COR_CAPSULA_2
    );
}
...
// ============================================================
// RESULTADO FINAL — Inicial / Final / Variação
// ============================================================
...
function classificarVariacao(variacao) {
    if (!numeroValido(variacao)) return "neutro";
    if (variacao > 0.05)  return "positivo";
    if (variacao < -0.05) return "negativo";
    return "neutro";
}

function formatarVariacao(variacao) {
    if (!numeroValido(variacao)) return "--";
    const sinal = variacao > 0 ? "+" : (variacao < 0 ? "−" : "±");
    return sinal + formatarNumero(Math.abs(variacao), 2) + " °C";
}

function blocoCapsula(numero, inicial, final, cor) {
    const variacao = (numeroValido(inicial) && numeroValido(final))
        ? (final - inicial)
        : null;

    const classeVar = classificarVariacao(variacao);

    return `
        <div class="capsula-resultado">
            <div class="capsula-header">
                <span class="capsula-badge" style="background:${cor}"></span>
                <strong>Cápsula ${numero}</strong>
            </div>

            <div class="capsula-metricas">
                <div class="metrica">
                    <span class="metrica-rotulo">Inicial</span>
                    <span class="metrica-valor">
                        ${numeroValido(inicial)
                            ? formatarNumero(inicial, 2) + '<small>°C</small>'
                            : '--<small>°C</small>'}
                    </span>
                </div>

                <div class="metrica">
                    <span class="metrica-rotulo">Final</span>
                    <span class="metrica-valor">
                        ${numeroValido(final)
                            ? formatarNumero(final, 2) + '<small>°C</small>'
                            : '--<small>°C</small>'}
                    </span>
                </div>

                <div class="metrica destaque">
                    <span class="metrica-rotulo">Variação</span>
                    <span class="metrica-valor ${classeVar}">
                        ${numeroValido(variacao)
                            ? formatarVariacao(variacao).replace(" °C", "") + '<small>°C</small>'
                            : '--<small>°C</small>'}
                    </span>
                </div>
            </div>
    ...
    areaResultado.innerHTML = `
        <div class="resultado-final">
            <div class="resultado-cabecalho">
                <div class="resultado-cabecalho-texto">
                    <span>RESULTADO FINAL</span>
                    <h3>Comparação das cápsulas</h3>
                </div>
                <div class="resultado-selo">Concluído</div>
            </div>

            <div class="resultado-capsulas">
                ${blocoCapsula(1, temperaturaInicialCapsula1, temperaturaFinalCapsula1, COR_CAPSULA_1)}
                ${blocoCapsula(2, temperaturaInicialCapsula2, temperaturaFinalCapsula2, COR_CAPSULA_2)}
            </div>
        </div>
    `;
}
...
function executarReprise() {
    ...
    const tempoDecorrido = (performance.now() - repriseInicio) / 1000;
...
function desenharGraficoReprise(tempoAtualReprise) {
    ...
    desenharLinha(d1, margem, gL, gA, repriseDuracaoTotal,
                  escalaY.minimo, escalaY.maximo, COR_CAPSULA_1);

    desenharLinha(d2, margem, gL, gA, repriseDuracaoTotal,
                  escalaY.minimo, escalaY.maximo, COR_CAPSULA_2);
}
